...
const Isi = () => {
  ...
  return (
    <div className="max-w-[80rem] mx-auto p-4">
      ...
      <form className="max-w-[100%] mx-auto mt-4" onSubmit={performSearch}>
          ...
          {results.length > 0 &&
            results.map((result) => (
              <div key={result.id} className="p-1">
                <img ref={setImageRef} src={result.urls.small} alt={result.description || 'Image'} className="rounded-lg w-full"/> 
              </div>
          ))}
        </Masonry>
      </div>
